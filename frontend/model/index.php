...
/*----------------------[ astrology_course ]----------------------*/
  ...
  function get_astrology_course_comma($course_id){
    global $conn;


    $sql = "SELECT * 
            FROM   tb_astrology_course
            WHERE status='เปิด'  AND id not in(".$course_id.")
            ORDER BY case when pin != 0 then 0 else 1 end, pin ASC,id DESC";
    return $conn->query($sql);
  }

  function get_astrology_course_limit_comma($course_id){
    global $conn;
  ...
  function get_astrology_course_limit(){
    global $conn;
...
/*----------------------[ holy_object ]----------------------*/
  ...
  function get_holy_object_limit(){
    global $conn;
    $sql = "SELECT * 
            FROM   tb_holy_object
            WHERE status='เปิด'
...
/*----------------------[ contact ]----------------------*/
  ...
  function insert_contact($post){
    global $conn;
    $sql = 'INSERT INTO tb_contact (name, email,topic, description,create_date,update_date) 
    VALUES ("'.$post['name'].'",
            "'.$post['email'].'",
            "'.$post['topic'].'",
            "'.$post['description'].'",
...
/*----------------------[ register ]----------------------*/
  ...
  function get_register_email($email){
    global $conn;
    $sql = 'SELECT * 
            FROM   tb_register
            WHERE email="'.$email.'" AND status="เปิด"';
    $result = $conn->query($sql);
  ...
  function get_register_email_duplicate($email){
    global $conn;
  ...
  function insert_register($post){
    global $conn;
    $sql = 'INSERT INTO tb_register (name,email,phone,sex, birthday,password,create_date,update_date) 
    VALUES ("'.$post['name'].'",
            "'.$post['email'].'",
            "'.$post['phone'].'",
            "'.$post['sex'].'",
  ...
  function update_register_member($post){
    global $conn;
    $sql = 'UPDATE  tb_register 
            SET     name        = "'.$post['name'].'",
                    sex         = "'.$post['sex'].'",
...
/*----------------------[ member_course ]----------------------*/
  ...
  function get_member_course_id_user_id($id,$user_id){
    global $conn;
    $sql = 'SELECT * 
            FROM   tb_member_course
  ...
  function get_member_course_id_user_ids($id,$user_id){
    global $conn;
    $sql = 'SELECT * 
            FROM   tb_member_course
            WHERE course_id="'.$id.'" AND user_id ="'.$user_id.'"';
  ...
  function get_member_course(){
    ...
    $sql = "SELECT r.name AS name_register,r.sex,r.birthday,r.email,r.phone,r.create_date AS create_date_register,r.update_date AS update_date_register,r.status AS status_register,
            m.user_id,m.course_id,m.slip_payment,m.status,m.create_date,m.update_date,
  ...
  function get_member_course_list($id){
    global $conn;
    $sql = "SELECT m.user_id,m.course_id,m.slip_payment,m.status,m.create_date,m.update_date,m.comment,
            a.id AS course_id,a.name AS name_course,a.description AS description_course,a.image AS image_course,a.video AS video_course,a.price AS price_course,a.status AS status_course,a.create_date AS create_date_course,a.update_date AS update_date_course
            FROM   tb_member_course m
            LEFT JOIN tb_astrology_course a ON a.id = m.course_id
            WHERE m.user_id = $id
  ...
  function insert_member_course($post){
    global $conn;
    $sql = 'INSERT INTO tb_member_course (user_id, slip_payment, course_id,status,create_date,update_date) 
    VALUES ("'.$post['user_id'].'",
            "'.$post['file_name'].'",
            "'.$post['course_id'].'",
            "'.$post['status'].'",
  ...
  function update_member_course_slip($post){
    global $conn;
    $sql = 'UPDATE  tb_member_course 
            ...
            WHERE   user_id="'.$post['user_id'].'" AND course_id="'.$post['course_id'].'"';
    if ($conn->query($sql) === TRUE) {
      return true;
    } else {
      return false;
...
/*-------------comment--------------------*/
  ...
  function get_comment($page,$id){
    global $conn;
    $sql = 'SELECT * 
            FROM   tb_comment
            WHERE page="'.$page.'" AND id_comment="'.$id.'"
            ORDER BY id DESC';
    return $conn->query($sql);
  }
  ...
  function insert_comment($post){
    global $conn;
    $sql = 'INSERT INTO tb_comment (name, comment,create_date,page,id_comment) 
    VALUES ("'.$post['name'].'",
            "'.$post['comment'].'",
            NOW(),
            "'.$post['page'].'",
